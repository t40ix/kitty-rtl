...
typedef enum { LIGATURE_UNKNOWN, INFINITE_LIGATURE_START, INFINITE_LIGATURE_MIDDLE, INFINITE_LIGATURE_END } LigatureType;
...
typedef union DecorationsKey {
    struct {
        uint8_t scale : 8, subscale_n : 8, subscale_d : 8, align : 8, multicell_y : 8, u1 : 8, u2 : 8, u3 : 8;
    };
    uint64_t val;
} DecorationsKey;
...
static uint64_t
hash_decorations_key(DecorationsKey k) {
    return vt_hash_integer(k.val);
}
static bool
cmpr_decorations_key(DecorationsKey a, DecorationsKey b) {
    return a.val == b.val;
}
...
sprite_position_for(FontGroup *fg, RunFont rf, glyph_index *glyphs, unsigned glyph_count, uint8_t ligature_index, unsigned cell_count) {
    ...
    SpritePosition *s = find_or_create_sprite_position(
        font->sprite_position_hash_table, glyphs, glyph_count, ligature_index, cell_count, rf.scale, subscale, rf.multicell_y, rf.align.val, &created);
    if (!s) {
        PyErr_NoMemory();
        return NULL;
    }
...
calculate_underline_exclusion_zones(pixel *buf, const FontGroup *fg, DecorationGeometry dg, FontCellMetrics scaled_metrics) {
    ...
    switch (OPT(underline_exclusion.unit)) {
        case 2: thickness = ((long)round((OPT(underline_exclusion).thickness * (fg->logical_dpi_x / 72.0)))); break;
        case 1: thickness = (unsigned)OPT(underline_exclusion).thickness; break;
        default: thickness = (unsigned)(OPT(underline_exclusion).thickness * thickness); break;
    }
    ...
    if (0) {
        display_glyph(buf, fg->fcm.cell_width, fg->fcm.cell_height);
        printf("\n");
    }
...
current_send_sprite_to_gpu(FontGroup *fg, pixel *buf, DecorationMetadata dec, FontCellMetrics scaled_metrics) {
    ...
    if (python_send_to_gpu_impl) {
        python_send_to_gpu(fg, ans, buf);
        return ans;
    }
    if (dec.underline_region.height && OPT(underline_exclusion).thickness > 0)
        calculate_underline_exclusion_zones(buf, fg, dec.underline_region, scaled_metrics);
    send_sprite_to_gpu((FONTS_DATA_HANDLE)fg, ans, buf, dec.start_idx);
    if (0) {
        printf("Sprite: %u dec_idx: %u\n", ans, dec.start_idx);
        display_glyph(buf, fg->fcm.cell_width, fg->fcm.cell_height);
        printf("\n");
    }
    return ans;
...
create_features_for_face(const char *psname, PyObject *features, FontFeatures *output) {
    ...
    for (size_t i = 0; i < count_from_opts; i++) { add_feature(output, &from_opts->features[i]); }
...
init_hash_tables(Font *f) {
    ...
    if (!f->sprite_position_hash_table) {
        PyErr_NoMemory();
        return false;
    }
...
adjust_metric(unsigned int *metric, float adj, AdjustmentUnit unit, double dpi) {
    ...
    int a = 0;
    switch (unit) {
        case POINT: a = ((long)round((adj * (dpi / 72.0)))); break;
        case PERCENT: *metric = (int)roundf((fabsf(adj) * (float)*metric) / 100.f); return;
        case PIXEL: a = (int)roundf(adj); break;
    }
...
#define A(which, dpi) adjust_metric(&fg->fcm.which, OPT(which).val, OPT(which).unit, fg->logical_dpi_##dpi);
    A(underline_thickness, y);
    A(underline_position, y);
    A(strikethrough_thickness, y);
    A(strikethrough_position, y);
    A(baseline, y);
#undef A
...
not_found:
    if (do_debug) {
        ...
        debug("is ");
        PyObject_Print((PyObject *)face, stderr, 0);
...
load_fallback_font(FontGroup *fg, const ListOfChars *lc, bool bold, bool italic, bool emoji_presentation) {
    ...
    if (PyLong_Check(face)) {
        ssize_t ans = fg->first_fallback_font_idx + PyLong_AsSsize_t(face);
        Py_DECREF(face);
        return ans;
    }
...
chars_as_utf8(const ListOfChars *lc, char *buf, size_t bufsz, char_type zero_char) {
    ...
    if (lc->count == 1) n = encode_utf8(lc->chars[0] ? lc->chars[0] : zero_char, buf);
        ...
        if (lc->chars[0] != '\t')
            for (unsigned i = 1; i < lc->count && n < bufsz - 4; i++) n += encode_utf8(lc->chars[i], buf + n);
    }
    buf[n] = 0;
    return n;
}
...
fallback_font(FontGroup *fg, const CPUCell *cpu_cell, const GPUCell *gpu_cell, const ListOfChars *lc) {
    ...
    if (bold) style += italic ? 3 : 2;
    else style += italic ? 1 : 0;
...
static ssize_t
font_for_cell(
    FontGroup *fg, const CPUCell *cpu_cell, const GPUCell *gpu_cell, bool *is_main_font, bool *is_emoji_presentation, TextCache *tc, ListOfChars *lc) {
    *is_main_font = false;
    *is_emoji_presentation = false;
    ...
    switch (lc->chars[0]) {
        case 0:
        case '\t':
        case IMAGE_PLACEHOLDER_CHAR: return BLANK_FONT;
        case 0x2500 ... 0x2573:
        case 0x2574 ... 0x259f:
        case 0x25d6 ... 0x25d7:
        case 0x25cb:
        ...
        case 0x2800 ... 0x28ff:
        case 0xe0b0 ... 0xe0bf:
        case 0xe0d6 ... 0xe0d7:   // powerline box drawing
        ...
        default:
            ...
            // scaled as well.
            if (lc->count == 1 && (lc->chars[0] == ' ' || lc->chars[0] == 0x2002 /* en-space */) && (!cpu_cell->is_multicell || cpu_cell->scale == 1))
                return BLANK_FONT;
            *is_emoji_presentation = has_emoji_presentation(cpu_cell, lc);
            ...
            switch (gpu_cell->attrs.bold | (gpu_cell->attrs.italic << 1)) {
                case 0: ans = fg->medium_font_idx; break;
                case 1: ans = fg->bold_font_idx; break;
                case 2: ans = fg->italic_font_idx; break;
                case 3: ans = fg->bi_font_idx; break;
            }
            if (ans < 0) ans = fg->medium_font_idx;
            if (!*is_emoji_presentation && has_cell_text((bool (*)(const void *, char_type))face_has_codepoint, (fg->fonts + ans)->face, false, lc)) {
                *is_main_font = true;
                return ans;
            }
...
box_glyph_id(char_type ch) {
    START_ALLOW_CASE_RANGE
    switch (ch) {
        case 0x2500 ... 0x25ff: return ch - 0x2500;             // IDs from 0x00 to 0xff
        case 0xe0b0 ... 0xee0b: return 0x100 + ch - 0xe0b0;     // IDs from 0x100 to 0xe5b
        case 0x2800 ... 0x28ff: return 0xf00 + ch - 0x2800;     // IDs from 0xf00 to 0xfff
        case 0x1fb00 ... 0x1fbae: return 0x1000 + ch - 0x1fb00; // IDs from 0x1000 to 0x10ae
        case 0x1fbce ... 0x1fbef: return 0x10af + ch - 0x1fbce; // IDs from 0x10af to 0x10e0 (34 chars)
        case 0x1cd00 ... 0x1cde5: return 0x1100 + ch - 0x1cd00; // IDs from 0x1100 to 0x11e5
        case 0x1cc1b ... 0x1cc2f: return 0x11e8 + ch - 0x1cc1b; // IDs from 0x11e8 to 0x11fc (21 chars)
        case 0x1cc30 ... 0x1cc3f: return 0x11fd + ch - 0x1cc30; // IDs from 0x11fd to 0x120c (16 chars)
        case 0x1ce16 ... 0x1ce19: return 0x120d + ch - 0x1ce16; // IDs from 0x120d to 0x1210 (4 chars)
        case 0x1ce51 ... 0x1ceaf: return 0x1211 + ch - 0x1ce51; // IDs from 0x1211 to 0x126e (95 chars)
        case 0xf5d0 ... 0xf60d: return 0x2000 + ch - 0xf5d0;    // IDs from 0x2000 to 0x203d
...
#define a global_glyph_render_scratch
    ...
    if (a.sz < sz) {
        free(a.glyphs);
        a.glyphs = malloc(sz * sizeof(a.glyphs[0]));
        if (!a.glyphs) fatal("Out of memory");
        free(a.sprite_positions);
        a.sprite_positions = malloc(sz * sizeof(SpritePosition *));
        if (!a.sprite_positions) fatal("Out of memory");
        a.sz = sz;
        if (!a.lc) {
...
effective_scale(RunFont rf) {
    ...
    if (rf.subscale_n && rf.subscale_d && rf.subscale_n < rf.subscale_d) { ans *= ((float)rf.subscale_n) / rf.subscale_d; }
    return ans;
}
...
#define apply_scaling(which_fg)                                                                            \
    ...
    if (vt_is_end(i)) {
        ...
        copy.fcm.cell_height = scaled_cell_height;
        ...
        sfd.fcm = copy.fcm;
        sfd.font_sz_in_pts = copy.font_sz_in_pts;
        sfd.fcm.cell_width = scaled_cell_width;
...
extract_cell_from_canvas(FontGroup *fg, unsigned int i, unsigned int num_cells) {
    ...
    for (unsigned int r = 0; r < fg->fcm.cell_height; r++, dest += fg->fcm.cell_width, src += stride)
        memcpy(dest, src, fg->fcm.cell_width * sizeof(fg->canvas.buf[0]));
...
calculate_regions_for_line(RunFont rf, unsigned cell_height, Region *src, Region *dest) {
    ...
    Region src_in_full_coords = *src;
    unsigned full_dest_height = cell_height * rf.scale;
    ...
    src->top += src_top_delta;
    src->bottom = src->bottom > src_bottom_delta ? src->bottom - src_bottom_delta : 0;
    ...
    dest->top = dest_top_delta;
    dest->bottom = cell_height > dest_bottom_delta ? cell_height - dest_bottom_delta : 0;
}
...
extract_cell_region(Canvas *canvas, unsigned i, Region *src, const Region *dest, unsigned src_width, FontCellMetrics unscaled_metrics) {
    src->left = i * unscaled_metrics.cell_width;
    src->right = MIN(src_width, src->left + unscaled_metrics.cell_width);
    unsigned sz;
...
index_for_decorations(FontGroup *fg, RunFont rf, Region src, Region dest, FontCellMetrics scaled_metrics) {
    const DecorationsKey key = {
        .scale = rf.scale,
        .subscale_n = rf.subscale_n,
        .subscale_d = rf.subscale_d,
        .align = rf.align.val,
        .multicell_y = rf.multicell_y,
        .u1 = 0,
        .u2 = 0,
        .u3 = 0};
...
#define sp global_glyph_render_scratch.sprite_positions
    ...
    for (unsigned i = 0, cnum = 0; i < num_glyphs; i++) {
        ...
        dest.left = i * scaled_metrics.cell_width + right_shift;
        dest.right = dest.left + scaled_metrics.cell_width;
        render_alpha_mask(fg->canvas.alpha_mask, fg->canvas.buf, &src, &dest, src.right, mask_stride, 0xffffff);
    }
    src.right = mask_stride;
    dest = src;
    dest.right = unscaled_metrics.cell_width * num_cells;
     ...
     * fg->fcm.cell_width, fg->fcm.cell_height, src.top, src.bottom, num_cells);*/
    for (unsigned i = 0; i < num_cells; i++) {
        ...
        /*printf("Sprite %u: pos: %u sz: (%u, %u)\n", i, sp[i]->idx, fg->fcm.cell_width, fg->fcm.cell_height); dump_sprite(b, fg->fcm.cell_width,
         * fg->fcm.cell_height);*/
    }
#undef sp
#undef failed
...
render_filled_sprite(pixel *buf, unsigned num_glyphs, FontCellMetrics scaled_metrics, unsigned num_scaled_cells) {
    if (num_scaled_cells > num_glyphs) {
        ...
        for (unsigned y = 1; y < scaled_metrics.cell_height; y++)
            memcpy(buf + scaled_metrics.cell_width * num_scaled_cells * y, buf, sizeof(buf[0]) * scaled_metrics.cell_width * num_scaled_cells);
...
render_group(
    ...
    const TextCache *tc,
    float scale,
    FontCellMetrics unscaled_metrics) {
#define sp global_glyph_render_scratch.sprite_positions
...
#define failed                                                                \
    ...
    for (unsigned i = 0, ligature_index = 0; i < num_cells; i++) {
        bool is_repeat_sprite =
            is_infinite_ligature && i > 1 && i + 1 < num_glyphs && glyphs[i] == glyphs[i - 1] && glyphs[i] == glyphs[i - 2] && glyphs[i] == glyphs[i + 1];
    ...
    if (global_glyph_render_scratch.lc->chars[0] == 0x2588) {
        ...
        for (unsigned i = 1; i < num_glyphs && is_only_filled_boxes; i++)
            if (global_glyph_render_scratch.glyphs[i] != box_glyph_id) is_only_filled_boxes = false;
    }
    ...
    if (is_only_filled_boxes) { // special case rendering of █ for tests
        ...
        ri.rendered_width = num_glyphs * scaled_metrics.cell_width;
        ...
        render_glyphs_in_cells(
            ...
            num_glyphs,
            canvas,
            scaled_metrics.cell_width,
            scaled_metrics.cell_height,
            num_scaled_cells,
            scaled_metrics.baseline,
            &was_colored,
            (FONTS_DATA_HANDLE)fg,
            &ri);
        ri.rendered_width = MIN(ri.rendered_width, ri.canvas_width);
    }
    // printf("num_cells: %u num_scaled_cells: %u num_glyphs: %u scale: %f unscaled: %ux%u scaled: %ux%u rendered_width: %d\n", num_cells, num_scaled_cells,
    // num_glyphs, scale, unscaled_metrics.cell_width, unscaled_metrics.cell_height, scaled_metrics.cell_width, scaled_metrics.cell_height, ri.rendered_width);
    if (canvas == scratch) {
        if (canvas_width != scaled_canvas_width) {
            ...
            for (unsigned y = 0; y < scaled_metrics.cell_height; y++) {
                ...
                if (scaled_canvas_width < canvas_width) memset(dest_row + scaled_canvas_width, 0, sizeof(pixel) * (canvas_width - scaled_canvas_width));
            }
            ri.canvas_width = canvas_width;
    ...
    if (num_cells == num_scaled_cells && scale == 1.f && !rendering_in_smaller_area) {
        ...
        Region src = {.bottom = scaled_metrics.cell_height, .right = scaled_metrics.cell_width * num_scaled_cells},
               dest = {.right = unscaled_metrics.cell_width};
        calculate_regions_for_line(rf, unscaled_metrics.cell_height, &src, &dest);
        DecorationMetadata dm = index_for_decorations(fg, rf, src, dest, scaled_metrics);
        ...
        for (unsigned i = 0; i < num_cells; i++) {
            if (!sp[i]->rendered) {
                pixel *b = extract_cell_region(&fg->canvas, i, &src, &dest, scaled_canvas_width, unscaled_metrics);
                /*printf("cell %u src -> dest: (%u %u) -> (%u %u)\n", i, src.left, src.right, dest.left, dest.right);*/
                sp[i]->idx = current_send_sprite_to_gpu(fg, b, dm, scaled_metrics);
                if (!sp[i]->idx) failed;
                /*dump_sprite(b, unscaled_metrics.cell_width, unscaled_metrics.cell_height);*/
...
is_special_glyph(glyph_index glyph_id, Font *font, CellData *cell_data) {
    ...
    if (!s.special_set) {
        bool is_special = cell_data->current_codepoint ? (glyph_id != glyph_id_for_codepoint(font->face, cell_data->current_codepoint) ? true : false) : false;
        s.special_set = 1;
        s.special_val = is_special;
...
shaped_glyphs_have_iosevka_join_names(hb_font_t *hbf) {
    char glyph_name[256];
    glyph_name[arraysz(glyph_name) - 1] = 0;
...
is_iosevka_lig_starter(LigatureType before, LigatureType current, LigatureType after) {
    return (current == R || (current == I && (after == L || after == M))) && !(before == R || before == M);
}

static bool
is_iosevka_lig_ender(LigatureType before, LigatureType current, LigatureType after) {
    return (current == L || (current == I && (before == R || before == M))) && !(after == L || after == M);
}
#undef L
#undef M
#undef R
...
group_iosevka(Font *font, hb_font_t *hbf, const TextCache *tc, ListOfChars *lc) {
    ...
    for (size_t i = G(glyph_idx); i < G(num_glyphs); i++) { ligature_types[i] = ligature_type_for_glyph(hbf, G(info)[i].codepoint, font->spacer_strategy); }
...
shape_run(
    ...
    uint8_t subscale = ((rf.subscale_n & 0xf) << 4) | (rf.subscale_d & 0xf);
    ShapedRun cached;
    if (!force_bidi_ltr && shaped_run_get(font->shaped_run_hash_table, first_cpu_cell, num_cells, tc, disable_ligature, OPT(force_ltr), key_scale, subscale, &cached)) {
    ...
    for (unsigned idx = 0; idx < G(group_idx) + 1; idx++) {
        ...
        if (group->first_cell_idx <= cursor_offset && cursor_offset < group->first_cell_idx + group->num_cells) {
            if (group->has_special_glyph && group_has_more_than_one_scaled_cell(group, scale)) {
                ...
                *left = group->first_cell_idx;
                *right = group->first_cell_idx + group->num_cells;
            }
            break;
        }
    }
    ...
    while (idx <= G(group_idx)) {
        ...
        if (group->num_glyphs) {
            ...
            render_group(
                ...
                rf,
                global_glyph_render_scratch.glyphs,
                group->num_glyphs,
                center_glyph,
                tc,
                scale,
...
test_shape(PyObject UNUSED *self, PyObject *args) {
    ...
    while (num < line->xnum && cell_has_text(line->cpu_cells + num)) {
        index_type width = 1;
        if ((c = line->cpu_cells + num)->is_multicell) { width = c->width * c->scale; }
        num += width;
    }
    PyObject *face = NULL;
    Font *font;
...
render_run(
    ...
    switch (rf.font_idx) {
        default:
            scale = shape_run(
                first_cpu_cell, first_gpu_cell, num_cells, &fg->fonts[rf.font_idx], rf, fg, disable_ligature_strategy == DISABLE_LIGATURES_ALWAYS, tc, lc);
        ...
        case MISSING_FONT:
            while (num_cells--) {
                first_gpu_cell->sprite_idx = MISSING_GLYPH;
                first_cpu_cell++;
                first_gpu_cell++;
            }
            break;
...
is_non_emoji_dingbat(char_type ch, CharProps cp) {
    switch (ch) {
        START_ALLOW_CASE_RANGE
        case 0x2700 ... 0x27bf:
        case 0x1f100 ... 0x1f1ff: return !cp.is_emoji; END_ALLOW_CASE_RANGE
    }
    return false;
}
...
run_fonts_are_equal(const RunFont *a, const RunFont *b) {
    return a->font_idx == b->font_idx && a->scale == b->scale && a->subscale_n == b->subscale_n && a->subscale_d == b->subscale_d &&
           a->align.val == b->align.val && a->multicell_y == b->multicell_y;
}
...
finalize(void) {
    ...
    if (global_glyph_render_scratch.lc) {
        cleanup_list_of_chars(global_glyph_render_scratch.lc);
        free(global_glyph_render_scratch.lc);
    }
    global_glyph_render_scratch = (GlyphRenderScratch){0};
    free(shape_buffer.codepoints);
    zero_at_ptr(&shape_buffer);
}
...
#define u(x) \
    ...
    else if (strcmp(which, "hollow_cursor") == 0) add_hollow_cursor((uint8_t *)PyBytes_AS_STRING(ans), fcm, dpi, dpi); else {
        Py_CLEAR(ans);
        PyErr_Format(PyExc_KeyError, "Unknown decoration type: %s", which);
    }
    return ans;
#undef u
}
...
current_fonts(PyObject *self UNUSED, PyObject *args) {
    ...
    if (os_window_id) {
        ...
        if (!os_window) {
            PyErr_SetString(PyExc_KeyError, "no oswindow with the specified id exists");
            return NULL;
        }
...
PyTypeObject ParsedFontFeature_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "kitty.fast_data_types.ParsedFontFeature",
...
pyrender_box_char(PyObject *self UNUSED, PyObject *args) {
    ...
    double dpi_x = 96., dpi_y = 96., scale = 1.;
...
init_fonts(PyObject *module) {
    ...
    if (harfbuzz_buffer == NULL || !hb_buffer_allocation_successful(harfbuzz_buffer) || !hb_buffer_pre_allocate(harfbuzz_buffer, 2048)) {
        PyErr_NoMemory();
        return false;
    }
